    ...
    liteJS.prototype = {
        ...
        run: function (hostSelector, dependencies, callback) {
            ...
            document.addEventListener("DOMContentLoaded", function () {
                ...
                initUIComponents(query(configService.hostSelector).find("[data-component]"));
...
(function () {
    ...
    router.provider("router", ["query", "config", "instanceFactory"], function (query, configService, instanceFactory) {
        ...
        function createComponentInstance(data) {
            ...
            component.attr("data-component", data.routeConfig.component);
            component.attr("data-init", JSON.stringify(data.params));
            component.addClass((data.routeConfig.animation || configService.defaultAnimation) || "");
            configService.keepComponentOnRouteChange ? ltView.append(component) : ltView.html(component);
            instanceFactory.initUIComponents(component);
            return component;
        }
        function renderLtView(data) {
            if (configService.keepComponentOnRouteChange) {
                if (lastComponent) {
                    lastComponent.css("display", "none");
                }
                if (componentsName.indexOf(data.routeConfig.component) == -1) {
                    componentsName.push(data.routeConfig.component);
                    lastComponent = createComponentInstance(data);
                } else {
                    lastComponent = ltView.find("[data-component=" + data.routeConfig.component + "]");
                }
